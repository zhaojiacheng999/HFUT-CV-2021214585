...
def process_image(image_path):
    # Load and process the image similar to MNIST
    image = cv2.imread(image_path, cv2.IMREAD_GRAYSCALE)
    image = cv2.bitwise_not(image)  # Invert colors
    image = cv2.resize(image, (280, 28))  # Assuming the image is a horizontal sequence of digits
    return image

def segment_image(image):
    # Segment the image into 10 parts
    segmented_images = [image[:, i*28:(i+1)*28] for i in range(10)]
    return segmented_images

def predict_digit(model, image):
    # Transform the image for the model input and predict
...
if __name__ == '__main__':
    ...
    """
    # 加载模型
    model = Net()  # 确保这是原始模型的相同结构
    model.load_state_dict(torch.load('model_mnist.pth'))
    #model.eval()  # 用于评估模式

    # Process the image
    image_path = 'assigment3_test.png'  # Replace with your image path
    processed_image = process_image(image_path)

    # Segment the image
    segmented_images = segment_image(processed_image)

    # Predict each digit
    predictions = [predict_digit(model, img) for img in segmented_images]

    # Visualize and print predictions
